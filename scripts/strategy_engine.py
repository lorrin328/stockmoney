...
class StrategyEngine:
    ...
    def _derive_sectors(self, phase: str, tech_drivers: List[str]) -> List[str]:
        """根据周期阶段推导关键赛道（2026年5月1日更新：地缘升级+信心建设战）"""
        base_sectors = {
            "复苏期": [
                "黄金 — 地缘避险首选+央行购金+去美元化（4·29美联储鸽派分歧+霍尔木兹封锁升级）",
                "工业金属（铜、铝）— 实物资产主线+新能源用铜+铜矿紧缺+美国关税预期",
                "AI算力基础设施（芯片、服务器、数据中心）— 十五五集成电路支柱+国产替代核心",
                "具身智能/机器人 — 十五五未来产业+AI应用落地+政治局信心建设",
                "创新药与高端医疗器械 — 十五五生物医药支柱+集采优化+创新支付",
                "高端制造替代（半导体设备、工业母机、工业软件）— 卡脖子攻关+首台套补贴",
                "新能源产业链（储能、光伏、电池）— 新型电力系统+储能临界点",
                "科创50/科技ETF — 十五五硬科技综合载体+康波核心赛道",
                "能化（油气、化工）— 霍尔木兹封锁推升原油至113美元，景气共振",
                "红利低波 — 高股息防御+新国九条鼓励分红",
            ],
            "繁荣期": [
                "科技成长股（全面享受泡沫）",
    ...
    def _derive_entry(self, phase: str, market_signal: str, resonance: str) -> str:
        ...
        if phase == "复苏期":
            if market_signal == "bullish":
                return "节后红五月窗口：左侧布局+右侧确认。5/6开盘若高开<1%可分批建仓，>2%等回踩；4%定投法触发同步执行"
            else:
    ...
    def _derive_risk_mgmt(self, phase: str, resonance: float, market: float) -> List[str]:
        """推导风险管理措施（2026年5月1日更新：地缘升级+信心建设战）"""
        rules = []
        ...
        # 流动性与避险（5/1更新：黄金升至12-15%底仓+实物资产主线）
        rules.append("始终保持10-15%现金（地缘风险+4%定投法需要现金储备）")
        rules.append("黄金配置12-15%作为避险底仓（4·29美军封锁霍尔木兹+美联储8:4分歧）")
        rules.append("有色金属（铜铝）配置5-8%，承接实物资产主线轮动")

        # 周期与地缘风险
        rules.append("康波周期判断可能滞后2-5年，保持灵活")
        rules.append("政策干预可能改变周期轨迹，关注央行动向（美联储7月可能换帅沃什）")
        rules.append("地缘政治黑天鹅：台海/中东突发事件时，果断减仓至30%以下")
        rules.append("中美关税升级时，增配国产替代（芯片、工业软件）+减持出口链")
        rules.append("原油持续>120美元时，启动通胀对冲：增配能化+减持高估值消费")
        rules.append("政治局'信心建设战'框架：节后红五月概率高，但若A股5/6开盘高开>2%则等回踩再买")
