...
FED_POLICY = {
    ...
    "next_meeting": "2026年6月16-17日",
    "2026_cuts_expected": 1,  # 官方预测
    "market_pricing_cuts": 0,  # 市场定价
    "qt_status": "已暂停",
    "qt_policy": "维持充足准备金",
    "balance_sheet": "平稳运行阶段",
    "inflation_target": "2%",
    "current_inflation": "3.3%",
    "chairman_change": "2026年5月鲍威尔任期届满",
    "chairman_candidates": ["Kevin Hassett（偏鸽）"],
}
...
class PolicyAnalyzer:
    ...
    def analyze_monetary_policy(self) -> PolicyFactor:
        ...
        return PolicyFactor(
            ...
            impact_score=score,
            confidence="medium",
            description="中国适度宽松（降准降息预期）vs 美联储维持高利率（全年0-1次降息），利差压力压制人民币资产",
            key_events=[
                f"美联储维持利率{self.fed['current_rate']}不变",
                f"缩表已暂停，转向{self.fed['qt_policy']}",
                f"中国1年期LPR {self.pbc['lpr_1y']}，政策立场：{self.pbc['policy_stance']}",
                f"美联储主席换届（5月），候选人偏鸽",
            ],
            affected_sectors=[
                "港股（流动性敏感）", "黄金", "高股息资产",
    ...
    def analyze_geopolitics(self) -> PolicyFactor:
        ...
        return PolicyFactor(
            ...
            impact_score=-5,
            confidence="medium",
            description="中美科技脱钩持续，但十五五规划国产替代加速，短期承压长期受益",
            key_events=[
                "中美关税战持续，半导体出口管制",
                "中东战争：霍尔木兹海峡封锁",
                "台湾海峡不确定性",
    ...
    def analyze_domestic_economy(self) -> PolicyFactor:
        ...
        return PolicyFactor(
            ...
            impact_score=5,
            confidence="medium",
            description="GDP增速目标5%左右，房地产触底，消费复苏，制造业投资回暖",
            key_events=[
                "房地产周期触底（2026-2027）",
                "设备投资周期启动（朱格拉复苏）",
                "库存周期补库存（基钦上行）",
            ...
            affected_sectors=[
                "银行ETF(512800)", "证券ETF(512880)", "沪深300ETF(510300)",
            ],
        )
